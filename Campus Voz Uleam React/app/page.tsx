...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      const list: Post[] = []
      snapshot.forEach((doc) => list.push({ id: doc.id, ...(doc.data() as Omit<Post, "id">) }))
      setPosts(list)
    })
    return () => unsub()
  }, [])

  // Show selected profile (searched)
  if (selectedProfile && user) {
    return (
      <ProfilePage
        user={user}
        userData={selectedProfile}
        publicationCount={posts.filter(post => post.uid === selectedProfile.uid).length}
  ...
  if (showProfile && user && userData) {
    return (
      <ProfilePage
        user={user}
        userData={userData}
        publicationCount={posts.filter(post => post.uid === user.uid).length}
        ...
        <div className="max-w-md w-full">
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/20 p-8 text-center">
            <div className="mb-8">
              ...
              <p className="text-sm text-gray-500">Comparte ideas, vota iniciativas y participa en el diálogo académico</p>
            </div>
            <button
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header
        ...
        onUserSelect={(profile) => setSelectedProfile(profile)}
        onLogout={() => signOut(auth)}
      />
      <Dashboard user={user} userData={userData} posts={posts} loadingUserData={loadingUserData} />
      {userData?.role === 'authority' && <ManageUsers />}
    </div>
  )
}